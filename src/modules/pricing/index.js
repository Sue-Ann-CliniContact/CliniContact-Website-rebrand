...
import { TIERS, ENTERPRISE, ALWAYS_INCLUDED, ENTRY_LABEL, MODELS, PRINCIPLE, MODELS_CONTRAST } from './pricing.data.js';
    ...
    <div class="ccpr-bg">
      <div class="ccpr-inner">
        ...
        <div class="ccpr-twoways">
          ${MODELS.map(
            (m) => `
            <div class="ccpr-tm${m.featured ? ' featured' : ''}">
              <span class="ccpr-tm-tag">${m.tag}</span>
              <h3 class="ccpr-tm-name">${m.name}</h3>
              <p class="ccpr-tm-desc">${m.desc}</p>
              <p class="ccpr-tm-who"><b>Best for</b> ${m.who}</p>
            </div>`
          ).join('')}
        </div>
        <p class="ccpr-contrast">${MODELS_CONTRAST}</p>
